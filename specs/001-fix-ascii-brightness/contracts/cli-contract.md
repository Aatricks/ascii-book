# CLI Contracts: Fix ASCII Brightness and Colors
...
**Purpose**: Generate ASCII art from input image with corrected brightness and colors
...
**Output**: 
- Success: Creates output image file with accurate brightness/colors, returns exit code 0
- Error: Prints error message to stderr, returns exit code 1

**Examples**:
```bash
# Basic ASCII art generation with brightness fix
...
**Output**: Image file containing ASCII art with corrected brightness and colors (binary)

**Logs**: Human-readable text to stdout/stderr, structured JSON available with --json flag</content>
<parameter name="filePath">c:\Dev\ascii-g\specs\001-fix-ascii-brightness\contracts\cli-contract.md
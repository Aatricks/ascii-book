# CLI Contracts: Fix Minimalistic Mode
...
**Purpose**: Generate ASCII art from input image with fixed minimalistic mode
...
- `--minimalistic`: Enable minimalistic mode with proper background and edge handling (optional)
...
**Output**: 
- Success: Creates output image file with corrected minimalistic mode, returns exit code 0
...
**Output**: Image file containing ASCII art with fixed minimalistic mode (binary)

**Logs**: Human-readable text to stdout/stderr, structured JSON available with --json flag</content>
<parameter name="filePath">c:\Dev\ascii-g\specs\001-fix-minimalistic\contracts\cli-contract.md